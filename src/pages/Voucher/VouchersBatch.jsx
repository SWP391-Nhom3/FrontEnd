...
import {
  // fetchGetVoucherType, fetchUpdateVoucher,fetchDeleteVoucher,
  fetchGetVoucher,
} from "../../data/api";
...
const VouchersBatch = () => {
  ...
  const handleViewVoucherBatch = (voucherBatchId) => {
    navigate(`/voucher-batch/${voucherBatchId}`);
  };
  ...
  const columns = [
    ...
    {
      title: "Tên lô giảm giá",
      dataIndex: "voucherBatchName",
      key: "voucherBatchName",
      render: (text) => <div className="text-base font-semibold">{text}</div>,
    },
    {
      title: "Loại Voucher",
    ...
    },
    {
      ...
      render: (text, record) => (
        ...
        >
          <Button
            type="link"
            onClick={() => handleViewVoucherBatch(record._id)}
            style={{
