...
import { CheckCircleOutlined, SmileOutlined, SyncOutlined } from '@ant-design/icons';
...
const CompleteOrder = () => {
    ...
    return (
      <div style={{ display: 'flex', justifyContent: 'center', height: '80vh' }}>
        <Card title="Đơn hoàn thành" style={{ width: '90%', marginTop: '50px', height: '75vh' }}>
          <div>
            <Table
              dataSource={orders.filter(item => item.orderStatus.name === "Hoàn thành")}
              rowKey={item => item.id}
              pagination={{
                current: currentPage,
                pageSize: pageSize,
                total: orders.filter(item => item.orderStatus.name === "Hoàn thành").length,
