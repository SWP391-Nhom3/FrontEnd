...
import { fetchBrands, fetchCategories, fetchProductsById, fetchUpdateProduct } from '../../data/api';
...
const EditProduct = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            console.error('Error during product submission:', error.response || error.message);
            notification.error({
                message: "Đã xảy ra lỗi trong quá trình cập nhật sản phẩm. Vui lòng thử lại sau!",
                placement: "top",
            })
        }
    };
                        ...
                        <Col span={18}>
                            ...
                            >
                                ...
                                {categories.map((option) => (
                                    <option key={option.id} value={option.name} selected={selectedCategoryId === option.id}>
                                        {option.name}
                                    </option>
                                ))}
                            </Select>
                    ...
                    <Row justify="space-around" align="middle" style={{ marginBottom: '40px', marginTop: '20px' }}>
                        ...
                        <Col span={18}>
                            ...
                            >
                                ...
                                {brands.map((option) => (
                                    <option key={option.id} value={option.name} selected={selectedBrandId === option.id} >
                                        {option.name}
                                    </option>
                                ))}
                            </Select>
